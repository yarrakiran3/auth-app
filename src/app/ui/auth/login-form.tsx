...
import {  handleGitLogin, handleGoogleLogin, handleLinkedInLogin, logIn } from '../../lib/actions'
import { LoginFormDataObject } from '../../lib/definitions';
import Link from 'next/link';
import { isValidEmail } from './signup-form';

export default function LoginForm() {
  const [formDataObject,setFormDataObject]=useState<LoginFormDataObject>({
    mail:"",
    password:""
  });
  let newerror;
  ...
  async function handleSubmit(formDataObject:LoginFormDataObject){

    if(!isValidEmail(formDataObject.mail)){
        setIthasError(true);
        newerror="Invalid Email"
        setErrorMessage(newerror);
          
    } else if(formDataObject.password.length<=5){
        newerror="Password is Invalid"
        setIthasError(true);
        setErrorMessage(newerror);
          
    } else {
            const resultOfSignIn= await logIn(formDataObject);
                if(resultOfSignIn==="Invalid credentials."){
                    setIthasError(true);
                    newerror="Invalid credentials"
                    setErrorMessage(newerror);
                    
                }else if(resultOfSignIn==="Something went wrong."){
                    setIthasError(true);
                    newerror="Something went wrong"
                    setErrorMessage(newerror);
                    
                }  else if(resultOfSignIn==="Invalid Credentials"){
                        setIthasError(true);
                        newerror="Invalid credentials"
                    setErrorMessage(newerror);
  ...
  return (
      ...
      <div className='flex flex-col gap-2 p-2 md:p-4'>
        <div >
          <label htmlFor='mail' className='p-2 font-bold'>Email</label><br></br>
            ...
            <button 
            className='flex flex-row justify-around bg-blue-500 p-2 mt-2 mr-4 rounded-xl bg-gradient-to-r from-lightblue via-middleblue to-darkblue text-white font-extrabold'
            onClick={()=>{handleSubmit(formDataObject)}}>Login</button>
            </div>
    ...
    <div className='text-center'>
                No account? <Link href={'/auth/signup'} className=' bg-gradient-to-r from-lightblue via-middleblue to-darkblue text-transparent bg-clip-text font-extrabold'>SignUp Here </Link>
    </div>
