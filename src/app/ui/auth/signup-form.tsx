...
import {  handleGitSignUp, handleGoogleSignUp, handleLinkedInSignUp, CredentialsSignUp } from '../../lib/actions'
import {  SignUpFormDataObject } from '../../lib/definitions';
import Link from 'next/link';

export default function SignUpForm() {
  const [formDataObject,setFormDataObject]=useState<SignUpFormDataObject>({
    name:"",
    mail:"",
    password:"",
    repassword:"",
  ...
  const [hasError,setIthasError]=useState(false);

  async function handleSubmit(formDataObject:SignUpFormDataObject){
    const name=formDataObject.name;
    const password=formDataObject.password.normalize()
    const repassword=formDataObject.repassword.normalize()

     if
     (formDataObject.name.length<=3||
        formDataObject.name===null||
        formDataObject.name===undefined)
     {
        setIthasError(true);
        const new_error="Enter a Valid Name"
        setErrorMessage(new_error)
     }else if
        (formDataObject.mail===null||
        !isValidEmail(formDataObject.mail)||
        formDataObject.mail===undefined )
        {
            setIthasError(true);
            const new_error="Enter a Valid email"
            setErrorMessage(new_error)
        }
        else if(password!==repassword)
            {
            setIthasError(true);
            const new_error="Passwords do not match"
            setErrorMessage(new_error)
        } 
        else if(password.length<=5||repassword.length<=5)
            {
                setIthasError(true);
                const new_error="Enter a Valid Password"
                setErrorMessage(new_error)
        } 
        else if(!isValidPassword(password)&&!isValidPassword(repassword))
            {
                    setPasswordIsNotValid(true);
        } 
        else{
            const email=formDataObject.mail;
            
                const resultOfSignIn= await CredentialsSignUp(name,email,password);

                if (resultOfSignIn==="Account Already Exists")
                    {
                    setIthasError(true);
                    const new_error="User already exists. Try Signing in"
                    setErrorMessage(new_error) 
                    }
        }
  ...
  function handleChange(e:any){
      ...
      let newDataObject={...formDataObject}
      newDataObject[name]=value;
      setFormDataObject(newDataObject);
  }
  ...
  return (
    ...
    <div className='bg-gray-200 rounded-xl shadow-xl p-12'>
      ...
      <div className='flex flex-col gap-2 p-1 '>
        <div >
        <label htmlFor='name' className='p-2 font-bold'>Name</label><br></br>
            <input 
            ...
            <br></br>
          <label htmlFor='mail' className='p-2 font-bold'>Email</label><br></br>
            <input 
            className='p-2 rounded-xl border-2 border-lightblue '
            type='mail' 
            name='mail' 
            ...
            <br></br>
            <p className='mt-2'></p>
            <label htmlFor='password' className='p-2 font-bold'>Password</label><br></br>
            <input 
            className='p-2 rounded-xl border-2 border-lightblue '
            type='password' 
            name='password' 
            ...
            <br></br>
            <p className='mt-2'></p>
            <label htmlFor='repassword' className='p-2 font-bold'>Re-enter Password</label><br></br>
            <input 
            className='p-2 rounded-xl border-2 border-lightblue '
            type='password' 
            name='repassword' 
            ...
            <br></br>
            {
              ...
              <p className='text-red-700 p-2 ml-2   '>{errorMessage}</p>
            }
            {
              passwordIsNotValid&&
              <ol className='text-red-700 px-2 mt-2'>
                <li>Password should contain</li>
                <li className='ml-4'>one lowercase</li>
                <li className='ml-4'>one uppercase letter</li>
                <li className='ml-4'>one number</li>
                <li className='ml-4'>Min. 6 characters</li>
              </ol>
            }
            <div className='flex flex-row justify-around mt-2'>
            <button onClick={()=>{handleSubmit(formDataObject)}} className='flex flex-row justify-around bg-blue-500 p-2 mt-2 mr-4 rounded-xl bg-gradient-to-r from-lightblue via-middleblue to-darkblue text-white font-extrabold'
      ...
      <div className='flex flex-col gap-2 p-2 md:p-4'>
          <form action={handleGoogleSignUp}>
                    <button
                  className="flex items-center bg-white border border-gray-300 rounded-lg shadow-md max-w-xs px-2 md:px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 w-full">
          ...
          <form action={handleLinkedInSignUp}>
                    <button
                  className="flex items-center bg-white w-full border border-gray-300 rounded-lg shadow-md max-w-xs px-2 md:px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                  <svg className="h-6 w-6 mr-2" 
                  ...
                  </svg> <span>SignUp with LinkedIn</span>
              </button>
          </form>

          <form action={handleGitSignUp}>
          
                <button
              className="flex items-center w-full bg-white border border-gray-300 rounded-lg shadow-md max-w-xs px-2 md:px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
              <svg className="h-6 w-6 mr-2" 
      ...
      <div className='text-center'>
                Already a user? <Link href={'/auth/signin'} className=' bg-gradient-to-r from-lightblue via-middleblue to-darkblue text-transparent bg-clip-text font-extrabold'>SignIn Here </Link>
    </div>
...
export function isValidPassword(password: string): boolean {
    const customPasswordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[A-Za-z\d@$!%*?&]{6,}$/;
